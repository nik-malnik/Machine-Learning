...
def pooling_layer_forward(input, layer):
  ...
  for n in range(batch_size):
    ...
    input_n = input_n.reshape((h_in, w_in, c))
    temp = np.zeros((h_out, w_out, c))
    for channel in range(c):
      for u in range(h_out):
        for v in range(w_out):
          temp[u,v,channel] = -1*float('inf')
          for e in range(k):
            for d in range(k):
              if input_n[u*stride + e,v*stride + d,channel] > temp[u,v,channel]:
                temp[u,v,channel] = input_n[u*stride + e,v*stride + d,channel]
          output['data'][:,n] = temp.reshape(h_out * w_out * c)
...
def pooling_layer_backward(output, input, layer):
  ...
  for n in range(batch_size):
    input_n = input['data'][:, n]
    input_n = input_n.reshape((h_in, w_in, c))
    output_n = output['data'][:,n].reshape((h_out, w_out, c))
    diff_n = output['diff'][:,n].reshape((h_out, w_out, c))
    temp_od = np.zeros(input_n.shape)
    for channel in range(c):
      for u in range(h_out):
        for v in range(w_out):
          for e in range(k):
            for d in range(k):
              if input_n[u*stride + e,v*stride + d,channel] == output_n[u,v,channel]:
                temp_od[u*stride + e,v*stride + d,channel] += diff_n[u,v,channel]
    input_od[:,n] = temp_od.reshape((h_in*w_in*c))
  # implementation ends
...
def relu_forward(input, layer):
  ...
  output['data'] = np.maximum(input['data'],output['data'])
...
def relu_backward(output, input, layer):
  ...
  # implementation begins
  for n in range(batch_size):
    input_od[:,n] = np.dot(np.diag(1*(output['data'][:,n] > 0)),output['diff'][:,n])
  # implementation ends
...
def inner_product_forward(input, layer, param):
  ...
  for n in range(batch_size):
    input_n = input['data'][:, n]
    output['data'][:,n] = np.dot(input_n.reshape(1,(len(input_n))),param['w']).flatten() + param['b']
  # implementation ends
...
def sgd_momentum(w_rate, b_rate, mu, decay, params, param_winc, param_grad):
  ...
  for i in range(1,len(params_)+1):
    param_winc_[i]['w'] = mu*param_winc[i]['w'] + w_rate*(param_grad[i]['w'] + decay*params[i]['w'])
